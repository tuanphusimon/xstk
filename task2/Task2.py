...
# -- Boxplot - contaminant_ppm by water_source_type ----
...
# -- Bar chart - mean contaminant_ppm per water source --------------------
...
# -- Scatter plot - contaminant_ppm vs. ph_level ----------------
...
print(f"{'Sample mean (mean_x)':<28} {x_well:>12.8f} {x_river:>12.8f}")
print(f"{'Sample std dev (s)':<28} {s_well:>12.8f} {s_river:>12.8f}")

# -- hypotheses -----------------------------------------------------------------
print("\nHypotheses:")
print("  H0: μ_Well = μ_River   (i.e. μ_Well - μ_River = 0)")
print("  H1: μ_Well ≠ μ_River   (two-tailed)")
...
print(f"\nCalculations:")
print(f"  Difference in means (mean_x_Well - mean_x_River) = {diff:+.8f}")
print(f"  Standard error of difference            = {se_diff:.8f}")
print(f"  Test statistic  z0                      = {z0:+.8f}")
print(f"  Critical value  z_{{alpha/2}} = z_0.025        = +/- {z_critical:.4f}")
...
print(f"\nConclusion:")
print(f"  At the 5 % significance level we {conclusion} H0.")
...
# -- visualise the two distributions + z statistic ---------------------
fig, axes = plt.subplots(1, 2, figsize=(13, 5))

# Left - overlapping histograms
ax = axes[0]
ax.hist(well,  bins=30, color=PALETTE["Well"],  alpha=0.65, edgecolor="white",
        label=f"Well  (n={n_well}, mean_x={x_well:.4f})", density=True)
ax.hist(river, bins=30, color=PALETTE["River"], alpha=0.65, edgecolor="white",
        label=f"River (n={n_river}, mean_x={x_river:.4f})", density=True)
...
ax.axvline( z_critical, color="#E53935", linewidth=1.5, linestyle=":",
            label=f"+/- z_{{alpha/2}} = +/- {z_critical:.2f}")
...
plt.tight_layout()
plt.savefig("B_two_sample_ttest.png", dpi=150, bbox_inches="tight")
plt.close()
...
print(f"  {'Grand mean (mean_x..)':<35} {grand_mean:>14.8f}")
print()
...
print()
print(f"  {'F0 = MSTr / MSE':<35} {F0:>14.8f}")
...
print(f"\nConclusion:")
print(f"  At alpha = 0.05 we {anova_decision} H0.")
...
ax.fill_between(x_rej, stats.f.pdf(x_rej, df_Tr, df_E),
                color="#E53935", alpha=0.35, label=f"Rejection region (alpha=0.05)")
...
plt.tight_layout()
plt.savefig("C_ANOVA.png", dpi=150, bbox_inches="tight")
plt.close()



# =============================================================================
# SECTION D - Chi-Square Test 1
# =============================================================================
...
ax.plot(x_c, y_c, color="#333", linewidth=2, label=f"X²(df={df2_chi})")
...
ax.annotate(f"X² = {chi2_2:.4f}\np = {p_chi2_2:.4f}\n-> Fail to Reject H0",
            ...
            fontsize=9, color="#1565C0",
            bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="#1565C0", alpha=0.9))
ax.set_title(f"X²-Distribution: Test 1 (Well vs. River)\ndf = {df2_chi}",
             fontsize=12, fontweight="bold")
...
plt.tight_layout()
plt.savefig("D_chisq_well_vs_river.png", dpi=150, bbox_inches="tight")
plt.close()


# =============================================================================
# SECTION E - Chi-Square Test 2: All 6 sources
# =============================================================================
...
print(f"\nConclusion:")
print(f"  At alpha = 0.05 we {e_decision} H0.")
...
# -- plot 1: heatmap of observed & contributions ----------------------------
fig, axes = plt.subplots(1, 2, figsize=(14, 5))
...
ax.plot(x_c2, y_c2, color="#333", linewidth=2, label=f"X²(df={df_all})")
...
ax.annotate(f"X² = {chi2_all:.4f}\np = {p_chi2_all:.4f}\n-> Fail to Reject H0",
            ...
            fontsize=9, color="#1565C0",
            bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="#1565C0", alpha=0.9))
ax.set_title(f"X²-Distribution: Test 2 (All 6 Sources)\ndf = {df_all}",
             fontsize=12, fontweight="bold")
ax.set_xlabel("X²"); ax.set_ylabel("Density")
ax.legend(fontsize=9); ax.grid(axis="y", linestyle="--", alpha=0.4)
ax.spines[["top","right"]].set_visible(False)
...
plt.tight_layout()
plt.savefig("E_chisq_all_sources.png", dpi=150, bbox_inches="tight")
plt.close()
...
print("  " + "-" * 80)
print(f"  {'Two-Sample t-Test (Well vs River)':<35} {f'z0={z0:.4f}':>12} {'+/- 1.96':>10} {f'{p_value:.4f}':>10}  Fail to Reject H0")
print(f"  {'ANOVA (6 sources)':<35} {f'F0={F0:.4f}':>12} {f'{F_crit:.2f}':>10} {f'{p_anova:.4f}':>10}  Fail to Reject H0")
print(f"  {'Chi-Square 1 (Well vs River)':<35} {f'X²={chi2_2:.4f}':>12} {'3.84':>10} {f'{p_chi2_2:.4f}':>10}  Fail to Reject H0")
print(f"  {'Chi-Square 2 (All 6 sources)':<35} {f'X²={chi2_all:.4f}':>12} {'11.07':>10} {f'{p_chi2_all:.4f}':>10}  Fail to Reject H0")
print()
print("  Overall: No test found a statistically significant difference in")
print("  contamination levels across any grouping at alpha = 0.05.")
print()
